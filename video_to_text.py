...
def batch_process_offline_videos(source_dir: str, output_root: str):
  """
  遍历指定目录，转录为文字
  ...
  for i,vid in enumerate(video_files, 1):
    ...
    final_text_file = current_dir / f"{vid.stem}_transcriped.txt"
    ...
    try:
      ...
      #默认第一个音轨
      raw_audio = extracted_audio[0]
      
      #2.提取人声
      logger.info(f"正在提取音频人声: {raw_audio}")
      vocal_audio = distractor(raw_audio,str(current_dir))
      if not vocal_audio:
        logger.warning(f"压缩失败:{raw_audio}")
        vocal_audio = raw_audio

      #3.压缩音频
      logger.info(f"正在压缩音频")
      compressed_audio = compresser(vocal_audio)

      if not compressed_audio:
        logger.warning(f"未能成功处理音频{vocal_audio}")
        compressed_audio = raw_audio
      #3.音频转文字
      result=processer.process_long_audio(compressed_audio)
...
if __name__ == "__main__":
  ...
  batch_process_offline_videos(E_DRIVE_PATH,RESULT_OUTPUT)
